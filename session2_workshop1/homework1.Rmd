...
ggplot(movies, aes(x=rating)) +
  ...
  theme_bw()
```
We observe that movies of Biography genre have the highest mean rating of 7.11. Movies belonging to Musical and Romance genres have the least variability in ratings (each having a standard deviation of 0.636). 
...
Let us first examine the rate at which employees leave the company. To do that we will create a density plot showing the probability that an employee will change jobs (for any reason) as it relates to his number of years within the company:
...
Going forward lets examine the distribution of the rates employees have given to their job satisfaction and work-life balance:
```{r}
library(scales)
##JOB SATISFACTION
...
Then we are going to examine how one's educational background, and gender relate to monthly income:
```{r}
...
The next thing we should examine when it comes to monthly incomes, is how different job roles are compensated within the company:
```{r}
hr_cleaned %>% 
...
Lets revert back to education level and how it relates to to income level. We will create a bar chart showing the median of salaries for different educational backgrounds. Median is preferred over mean for salaries as it is more robust to outliers.
```{r}
hr_cleaned %>% 
  group_by(education) %>% 
...
```
As what have been interpreted before, people are more likely to have higher monthly wages if they have been educated for a longer period. Besides, all these five density plots are right-skewed, so there are other factors that help to differentiate earnings between people having similar education backgrounds. The plot also looks better when we use the economist graph format.

Finally let us examine how income is distributed across different ages, for every different job role:
